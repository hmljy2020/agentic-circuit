...
generate_and_link_cpp() {
  ...
  --project-name=acpy-ring-noc --project-identity=project.chao.acpy-ring-noc \
  --system-name=ring_noc --system-identity=system.chao.ring-noc \
...
compile_runner() {
  ...
  /usr/bin/c++ -std=c++20 -I"${out_root}/include" -I"${repo_root}/include" \
    ...
    -L/usr/lib/llvm-22/lib -lLLVM -o "${out_root}/bin/ring-noc"
}
...
run_stage runtime "${out_root}/bin/ring-noc"
print_timing_summary
