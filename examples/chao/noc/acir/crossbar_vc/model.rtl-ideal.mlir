// Self-contained ACSim-ready variant of model.mlir.  Only the centralized
// scheduler differs: it uses compiler-native ac.arbitrate + ac.try_transfer.
//
...
builtin.module attributes {ac.contract_epoch = "0.2"} {
  ...
  ac.module @Crossbar() parameters {} graph {
    ...
    ac.process @scheduler kind "control" {
      ...
      // fire = enable && source.readable && destination.writable.  Here each
      // enable is the corresponding grant. On fire,
      ...
      %f1 = ac.try_transfer @in1_A to @out0_A when %g1 : i32
      %f2 = ac.try_transfer @in0_A to @out1_A when %g2 : i32
      %f3 = ac.try_transfer @in1_A to @out1_A when %g3 : i32
      %f4 = ac.try_transfer @in0_B to @out0_B when %g4 : i32
      %f5 = ac.try_transfer @in1_B to @out0_B when %g5 : i32
      %f6 = ac.try_transfer @in0_B to @out1_B when %g6 : i32
      %f7 = ac.try_transfer @in1_B to @out1_B when %g7 : i32
    ...
    ac.process @sink0 kind "control" {
      %a, %got_a = ac.try_recv @out0_A : i32
      %b, %got_b = ac.try_recv @out0_B : i32
      ac.yield_sim
    }

    ac.process @sink1 kind "control" {
      %a, %got_a = ac.try_recv @out1_A : i32
      %b, %got_b = ac.try_recv @out1_B : i32
      ac.yield_sim
    }
