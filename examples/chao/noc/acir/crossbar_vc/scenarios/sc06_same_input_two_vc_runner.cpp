...
int main() {
  ...
  // sc06 (test 6): A and B are both ready on input 0 with different (free)
  // destinations, yet only one leaves in a cycle -- A on epoch 1, B not until
  // epoch 2 once the input is free. Each epoch has exactly one transfer.
  const bool passed =
      result.classification == gfsim::TerminationClass::Incomplete &&
  ...
  std::cout << "sc06_passed=" << (passed ? "true" : "false") << '\n';
  return passed ? 0 : 1;
}
