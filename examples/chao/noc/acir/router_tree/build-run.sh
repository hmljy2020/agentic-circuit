...
# Keep the complete sequential build below the workstation's DRAM ceiling.
ulimit -v 1500000
...
"${repo_root}/build/dev-llvm22/bin/acir-opt" --verify-each=false \
  --pass-pipeline='builtin.module(ac-verify-model,ac-canonicalize-model,ac-freeze-topology)' \
  "${example_root}/model.mlir" -o "${build_root}/model.frozen.mlir"
...
"${repo_root}/build/dev-llvm22/bin/acir-cxxgen" \
  ...
  --project-name=chao-router-tree \
  --project-identity=project.chao.router-tree \
  --system-name=router_tree_demo \
  --system-identity=system.router-tree-demo \
...
if [[ ${#generated_objects[@]} -eq 0 ]]; then
  echo "no generated objects found" >&2
  exit 1
fi
...
/usr/bin/c++ -std=c++20 -I"${out_root}/include" \
  ...
  -L/usr/lib/llvm-22/lib -lLLVM -o "${out_root}/bin/router-tree-demo"

"${out_root}/bin/router-tree-demo"
