// Scenario sc06 (test 6): one physical input, two ready VCs, one cycle -> one
// grant. in0_A (dst0) and in0_B (dst1) are both valid and both of their output
// VCs (out0_A, out1_B) are empty. Even so, the per-input rule lets at most one
// flit leave physical input 0 per cycle: A is granted on tick 1 (A > B), B is
// blocked by the used-input flag and moves only on tick 2 once input 0 is free.
//
// Expected: in0_A.completed == 1, in0_B.completed == 1, out0_A.accepted == 1,
// out1_B.accepted == 1, and -- with no sinks -- epoch 1 has exactly ONE
// completed observation (the single grant), despite two eligible flits.
// Flits: A = dst0 vc0 src0 seq0 (0); B = dst1 vc1 src0 seq0 (4101).
...
builtin.module attributes {ac.contract_epoch = "0.2"} {
  ...
  ac.system @sc06_demo root @Crossbar as "root" tick 0 "cycle"
  ...
  ac.module @Crossbar() parameters {} graph {
    ...
    ac.process @scheduler kind "control" {
      ...
      %eb_b = arith.andi %eb_a, %w1b : i1
      %eb = arith.andi %eb_b, %d0b_is1 : i1
      // per-input rule: A and B cannot both leave input 0 in one cycle, even
      // though their destinations are different outputs, both currently free.
      %two_in = arith.andi %ea, %eb : i1
      %two_in_ok = arith.xori %two_in, %true : i1
      ac.assert %two_in_ok, "sc06: at most one flit per physical input per cycle"
      scf.if %ea {
        %s0 = ac.try_send @out0_A %h0a : i32
        ac.assert %s0, "sc06: A send must be accepted"
        scf.if %s0 {
          %v0, %g0 = ac.try_recv @in0_A : i32
          ac.assert %g0, "sc06: A grant must receive"
          %m0 = arith.cmpi eq, %v0, %h0a : i32
          ac.assert %m0, "sc06: A receive must match peek"
        }
      }
      scf.if %eb {
        %s0 = ac.try_send @out1_B %h0b : i32
        ac.assert %s0, "sc06: B send must be accepted"
        scf.if %s0 {
          %v0, %g0 = ac.try_recv @in0_B : i32
          ac.assert %g0, "sc06: B grant must receive"
          %m0 = arith.cmpi eq, %v0, %h0b : i32
          ac.assert %m0, "sc06: B receive must match peek"
        }
      }
      ac.yield_sim
    }
