...
// RUN: grep -R "const std::uint32_t start = static_cast<std::uint32_t>" %t.out/include/generated/processes
// RUN: grep -R "std::uint32_t next = start" %t.out/include/generated/processes
// RUN: grep -R "const std::uint32_t candidate = (start + offset) % count" %t.out/include/generated/processes
// RUN: %not grep -R -E "unordered_map|std::vector|while \(|malloc|operator new" %t.out/include/generated/processes %t.out/src/generated/processes
