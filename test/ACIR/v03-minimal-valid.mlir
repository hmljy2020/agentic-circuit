...
builtin.module attributes {ac.contract_epoch = "0.3"} {
  ...
  ac.module @minimal() parameters {} graph {
    %source = ac.source "input" : !ac.queue<!ac.struct<@types::@Input>, #ac.queue_contract<depth = 1, latency = 1, rate = 1, domain = @core, ordering = fifo>>
    %result = ac.compute %source {
    ^bb0(%record: !ac.var<!ac.struct<@types::@Input>>):
      %value = ac.var.get %record ["value"] : (!ac.var<!ac.struct<@types::@Input>>) -> !ac.var<i16>
    ...
    ac.observe %result as "result" fields ["value"] : !ac.queue<!ac.struct<@types::@Output>, #ac.queue_contract<depth = 1, latency = 1, rate = 1, domain = @core, ordering = fifo>>
    ac.observe %result as "result_copy" fields [] : !ac.queue<!ac.struct<@types::@Output>, #ac.queue_contract<depth = 1, latency = 1, rate = 1, domain = @core, ordering = fifo>>
    ac.return
  }
}
